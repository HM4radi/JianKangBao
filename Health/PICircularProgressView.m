...
- (void)drawRect:(CGRect)rect
{
    ...
    if (_showText && _textColor)
    {
        ...
        CGFloat fontSize2 = radius*0.16;
        CGFloat fontSize3 = radius*0.18;
        UIFont *font = [_font fontWithSize:fontSize];
        UIFont *font2 = [_font fontWithSize:fontSize2];
        UIFont *font3 = [_font fontWithSize:fontSize3];
        ...
        if (title) {
            [title drawAtPoint:CGPointMake((self.bounds.size.width/2)-22, (self.bounds.size.height/5)) forWidth:44 withFont:font3 lineBreakMode:NSLineBreakByWordWrapping];
        }
        
        if (goal) {
            [goal drawAtPoint:CGPointMake((self.bounds.size.width/2)-[goal length]*4.25, (self.bounds.size.height*0.8-8.5)) forWidth:[goal length]*8.5 withFont:font2 lineBreakMode:NSLineBreakByWordWrapping];
        }
    }
